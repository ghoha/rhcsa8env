...
config.vm.define "server2" do |server2|
  ...
  server2.vm.provision :shell, :inline => "/bin/sh /vagrant/networkexam.sh", run: "always"
...
config.vm.define "repo" do |repo|
  ...
  repo.vm.provision :shell, :inline => "yum install https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm -y; sudo yum install -y sshpass python3-pip python3-devel httpd sshpass vsftpd createrepo firewalld", run: "always"
...
config.vm.define "server1" do |server1|
  ...
  server1.vm.provision :shell, :inline => "/bin/sh /vagrant/networkexam.sh", run: "always"
